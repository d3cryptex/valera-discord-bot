...
interface BattlePassProgressParams {
    bot: DiscordBot;
    userId: string;
    guildId: string;
    type: 'messages' | 'voice' | 'gift' | 'invites' | 'streak' | 'custom' | 'game';
    gameType?: string;
    amount?: number;           // сколько прогресса добавить (по умолчанию 1)
    isDaily?: boolean;         // true — daily, false — weekly
    questIndex?: number;       // индекс квеста, если нужен (например, для вычисления ключа)
    messageContent?: string;   // для custom/streak (необязательно)
    channel?: any;             // канал для отправки уведомлений (опционально)
    interaction?: any;
}

export async function updateBattlePassProgress(params: BattlePassProgressParams): Promise<void> {
    const {
        bot, userId, guildId, type, amount = 1, isDaily = true,
        questIndex, messageContent = '', channel, interaction
    } = params;
    ...
    let bpDailyStatus: boolean[] = [];
    let bpWeeklyStatus: boolean[] = [];
    try { bpDailyStatus = JSON.parse(user.battlepass_daily_status ?? '[]'); } catch {}
    try { bpWeeklyStatus = JSON.parse(user.battlepass_weekly_status ?? '[]'); } catch {}

    let dailyCounters: Record<string, number> = {};
    let weeklyCounters: Record<string, number> = {};
    try { dailyCounters = JSON.parse(user.battlepass_progress_counters_daily || '{}'); } catch {}
    try { weeklyCounters = JSON.parse(user.battlepass_progress_counters_weekly || '{}'); } catch {}
    ...
    for (let i = 0; i < quests.length; i++) {
        ...
        if (statusArr[i]) continue; // если выполнено
        ...
        if (['messages','voice','gift','invites'].includes(type)) {
            ...
            if (quest.gameType && quest.gameType !== params.gameType) continue;
    ...
    if (newLevel > oldLevel) {
        for (let lvl = oldLevel + 1; lvl <= newLevel; lvl++) {
            const rewardObj = rewards.find((r: any) => r.level === lvl);
            if (rewardObj) {
                switch (rewardObj.type) {
                    case 'coins':
                        ...
                        if (channel && 'send' in channel && typeof channel.send === 'function') {
                            await channel.send(
                              `<@${userId}>, за BP-уровень ${lvl} начислено ${rewardObj.reward} монет!`
                            );
                        }
                        break;
                    ...
                    case 'role': {
                        ...
                        if (role) {
                            ...
                            if (member && !member.roles.cache.has(role.id)) {
                                ...
                                if (channel && 'send' in channel && typeof channel.send === 'function') {
                                    await channel.send(
                                      `<@${userId}>, вам выдана роль за BP-уровень ${lvl}: <@&${role.id}>`
                                    );
                                }
                            }
                        }
        ...
        await bot.database.User!.update(
            { battle_pass_level: newLevel, coins: user.coins },
            { where: { id: userId } }
        );
    }
}

async function processBattlePassLevelUp(user: any, settings: any, interaction: any, bot: DiscordBot) {
    ...
    while (user.battle_pass_exp >= bpXP && user.battle_pass_level < bpLevels) {
      ...
      const rewardObj = rewardsList.find((r: any) => r.level === user.battle_pass_level) ?? {};
