...
(0, canvas_1.registerFont)(path_1.default.join(__dirname, '../../fonts/impact.ttf'), { family: 'Impact' });
const emotionRules = [
...
class MarkovService {
    database;
    settingsService;
    customEmojiRe = /<a?:\w{2,}:\d{17,20}>/g; // для поиска в тексте
    customEmojiTokenRe = /^<a?:\w{2,}:\d{17,20}>$/; // для проверки целого токена
    uniEmojiRe = (0, emoji_regex_1.default)(); // глобальный поиск Unicode-эмодзи
    uniEmojiTokenRe = new RegExp(`^(?:${(0, emoji_regex_1.default)().source})$`); // проверка целого токена
    ...
    fitFontSize(ctx, text, maxWidth, baseSize, fontFamily, minSize = 14) {
    ...
    async makeMultiImageMeme(imagePaths, topText, bottomText) {
        imagePaths = imagePaths.filter(p => p && typeof p === 'string' && fs_1.default.existsSync(p) && p.length > 0);
        if (!imagePaths.length)
            throw new Error('Нет валидных изображений для мема');
        const images = await Promise.all(imagePaths.map(p => (0, canvas_1.loadImage)(p)));
    ...
    async makeOverlayMeme(mainPath, overlayPaths, topText, bottomText) {
        if (!mainPath || !fs_1.default.existsSync(mainPath))
            throw new Error('Главное изображение отсутствует: ' + mainPath);
        // Грузим главное изображение
        const mainImg = await (0, canvas_1.loadImage)(mainPath);
        // Создаем холст по размеру главной картинки
        const canvas = (0, canvas_1.createCanvas)(mainImg.width, mainImg.height);
        const ctx = canvas.getContext('2d');
        ...
        for (const path of overlayPaths) {
            const overlay = await (0, canvas_1.loadImage)(path);
    ...
    async sendAIReply(message, aiText) {
        try {
            ...
            if (canSend && r < 0.07) { // например, 7% шанс на гифку отдельно
                ...
                const gifMetas = await this.database.MemeImage.findAll({
                    where: {
                        ...
                        original_url: { [sequelize_1.Op.like]: '%.gif' }
                    },
                    raw: true
                });
            ...
            if (canSend && r < 0.13) {
                ...
                if (imagePaths.length === 0) {
                    ...
                    const attachment = new discord_js_1.AttachmentBuilder(buffer, { name: 'meme.png' });
                    ...
                    if (Math.random() < overlayChance) {
                        ...
                        const attachment = new discord_js_1.AttachmentBuilder(buffer, { name: 'meme_overlay.png' });
                        await message.channel.send({ files: [attachment] });
                        return;
                    }
                    else {
            ...
            logger_1.logger.error('sendAIReply error:', error);
        }
    }
    async reactToMessageSmart(message) {
        try {
            ...
            let pos = 0, neg = 0;
            if (/(супер|молодец|отлично|удачно|мило|клево|good|nice|amazing|great|happy|изи|gj)/i.test(msg))
                pos++;
            if (/(бред|дефект|ошибка|fail|фейл|печаль|грусть|trouble|bad|poor|слабо|печально|жесть|капец|wtf)/i.test(msg))
                neg++;
            if (pos > neg)
                await message.react('😃');
            else if (neg > pos)
            ...
            logger_1.logger.error('reactToMessageSmart error:', error);
        }
    }
    STOPWORDS = new Set(['и', 'а', 'но', ',', '.', '!', '?', '-', '']);
    async trainFromMessage(guildId, channelId, content) {
        const settings = await this.settingsService.getGuildSettings(guildId);
        if (!settings.ai.enabled)
            return;
        if (!this.settingsService.shouldProcessAI(settings, channelId))
            return;
        try {
            ...
            logger_1.logger.debug(`Trained Markov bigram chain with ${tokens.length} tokens from guild ${guildId}`);
        }
        catch (error) {
            logger_1.logger.error('Error training Markov bigram chain:', error);
        }
    }
    async getRandomBigramStart(guildId) {
        const candidates = await this.database.getBigramStartCandidates(guildId);
        // фильтруем только по стоп-словам!
        const filtered = candidates.filter(([prev, curr]) => !this.STOPWORDS.has(curr) &&
    ...
    async generateResponse(guildId, channelId) {
        const settings = await this.settingsService.getGuildSettings(guildId);
        if (!settings.ai.enabled)
            return null;
        if (!this.settingsService.shouldProcessAI(settings, channelId))
        ...
        try {
            ...
            for (let i = 0; i < maxWords - 2; i++) {
                ...
                if (this.STOPWORDS.has(next))
                    break;
                tokens.push(next);
                prev = curr;
                curr = next;
                if (this.isEndWord(next))
                    break;
            }
            ...
            logger_1.logger.error('Error generating Markov bigram response:', error);
            return null;
        }
    }
    tokenize(text) {
        ...
        const pattern = new RegExp(`${this.customEmojiRe.source}|${this.uniEmojiRe.source}|[A-Za-zА-Яа-яЁё0-9]+|[.,!?-]+`, 'g');
        const tokens = cleaned.match(pattern) ?? [];
        // 3) Приводим к нижнему регистру только «слова», эмодзи не трогаем
        return tokens.map(t => (this.customEmojiTokenRe.test(t) || this.uniEmojiTokenRe.test(t)) ? t : t.toLowerCase());
    }
    weightedRandomChoiceBigram(options) {
...
exports.MarkovService = MarkovService;
//# sourceMappingURL=MarkovService.js.map