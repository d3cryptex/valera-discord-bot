...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .setRequired(true)
    .addChoices({ name: 'XP за неделю', value: 'week' }, { name: 'XP за месяц', value: 'month' }, { name: 'XP за всё время', value: 'all' }, { name: 'Монеты (за всё время)', value: 'coins' }, { name: 'Голосовой чат (общее время)', value: 'voice' })))
    .addSubcommand(subcommand => subcommand
    .setName('addxp')
...
async function execute(interaction, bot) {
    ...
    switch (subcommand) {
        ...
        case 'leaderboard':
            ...
            if (type === 'week')
                await showXPLeaderboard(interaction, bot, 'week');
            else if (type === 'month')
                await showXPLeaderboard(interaction, bot, 'month');
            else if (type === 'all')
                await showXPLeaderboard(interaction, bot, 'all');
            else if (type === 'coins')
                await showCoinLeaderboard(interaction, bot);
            else if (type === 'voice')
                await showVoiceLeaderboard(interaction, bot);
            break;
        case 'addhp':
...
async function changeUserXp(interaction, bot, mode) {
...
async function changeUserCoins(interaction, bot, mode) {
    const db = bot.database;
    if (!db.User) {
        const embed = new discord_js_1.EmbedBuilder()
            .setColor('#eb4034')
...
async function showUserLevel(interaction, bot, userId) {
...
async function showXPLeaderboard(interaction, bot, period) {
    await interaction.deferReply();
    ...
    if (period === 'week') {
        ...
        topUsers = await bot.database.User.findAll({
            where: { updatedAt: { [sequelize_1.Op.gte]: from } },
...
async function showCoinLeaderboard(interaction, bot) {
    await interaction.deferReply();
    if (!bot.database.User) {
        const embed = new discord_js_1.EmbedBuilder()
            .setColor('#eb4034')
...
async function showVoiceLeaderboard(interaction, bot) {
    await interaction.deferReply();
    if (!bot.database.User) {
        const embed = new discord_js_1.EmbedBuilder()
            .setColor('#eb4034')
