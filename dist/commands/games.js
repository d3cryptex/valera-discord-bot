...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .addSubcommand(s => s.setName('dice')
    .setDescription('Бросить 2 кубика')
    .addIntegerOption(o => o.setName('bet').setDescription('Ставка (необязательно)').setRequired(false)))
    .addSubcommand(s => s.setName('coin')
...
const gameUserBets = new Map();
function isUserInPendingDuel(userId) {
    for (const duel of pendingDuels.values()) {
        if (duel.p1 === userId || duel.p2 === userId)
            return true;
    }
    return false;
}
async function playSlots(interaction, bot) {
    let bet = 0;
    ...
    const resultEmbed = new discord_js_1.EmbedBuilder()
        ...
        .setTitle('🎰 Результат')
        .setDescription(desc);
    const againButton = new discord_js_1.ActionRowBuilder()
...
async function playDice(interaction, bot) {
    let bet = 0;
    let userId = interaction.user.id;
    // Для ChatInputCommandInteraction найдём ставку через options
    if (interaction.isChatInputCommand()) {
...
async function playCoin(interaction, bot) {
    let bet = 0;
    let userId = interaction.user.id;
    // Для ChatInputCommandInteraction найдём ставку через options
    if (interaction.isChatInputCommand()) {
...
async function playDuel(interaction, bot, opponent) {
    ...
    const row = new discord_js_1.ActionRowBuilder()
        .addComponents([
        new discord_js_1.ButtonBuilder().setCustomId(`duel_accept:${duelId}`).setStyle(discord_js_1.ButtonStyle.Success).setLabel('Принять'),
        new discord_js_1.ButtonBuilder().setCustomId(`duel_decline:${duelId}`).setStyle(discord_js_1.ButtonStyle.Danger).setLabel('Отклонить')
    ]);
    await interaction.reply({ embeds: [embed], components: [row] });
}
async function startDuelBattle(interaction, duelId, p1id, p2id, bet, bot) {
    const hp = { [p1id]: 3, [p2id]: 3 };
    let active = p1id;
    let turn = 1;
    await interaction.message.edit({ components: [] });
    let msg = interaction.message;
    await msg.edit({ embeds: [new discord_js_1.EmbedBuilder()
...
const activeGameCollectors = new Map();
// Функция для создания коллектора кнопок для игр
async function setupGameButtonCollector(interaction, bot) {
...
async function handleGameButtonInteraction(interaction, bot) {
    ...
    if (customId.startsWith('duel_accept:')) {
        ...
        if (!duel || interaction.user.id !== duel.p2)
            return;
    ...
    if (customId.startsWith('duel_decline:')) {
        ...
        await interaction.message.edit({
            embeds: [new discord_js_1.EmbedBuilder().setTitle('Дуэль отклонена').setDescription(`<@${duel.p2}> отказался!`)],
            components: []
        });
        return;
    }
