...
async function execute(interaction, bot) {
    ...
    const hasPermission = await settingsService.hasPermission(interaction.member, settings);
    if (!hasPermission) {
...
async function addShopItem(interaction, bot) {
...
async function removeShopItem(interaction, bot) {
    await interaction.deferReply({ ephemeral: true });
...
async function listAllShopItems(interaction, bot) {
    await interaction.deferReply({ ephemeral: true });
    try {
        if (!bot.database.ShopItems)
            throw new Error('Database not connected');
...
async function toggleShopItem(interaction, bot) {
    await interaction.deferReply({ ephemeral: true });
    const itemId = interaction.options.getInteger('item_id', true);
    try {
        if (!bot.database.ShopItems)
