...
exports.data = new discord_js_1.SlashCommandBuilder()
    ...
    .setRequired(false)
    .addChoices(...SECTIONS.map(section => ({ name: section.name, value: section.value }))));
function twoCol(fields) {
    const result = [];
...
async function execute(interaction, bot) {
    ...
    if (section) {
        ...
        const embed = new discord_js_1.EmbedBuilder()
            ...
            .addFields(sectionData.commands.map((c) => ({
            name: c.description,
            value: c.usage,
            inline: false,
        })))
    ...
    const fieldArr = baseKeys.map(k => {
        ...
        return {
            ...
            value: sectionData.desc.length > 0
                ? sectionData.desc
                : sectionData.commands.map((c) => c.usage).slice(0, 2).join('\n'),
        };
    });
    const embed = new discord_js_1.EmbedBuilder()
        .setColor('#0099FF')
